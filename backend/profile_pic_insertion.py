...
def profile_pic_insertion():
    ...
    profile_pic_insertion = ("INSERT INTO SeniorProject.profile_picture VALUES ((SELECT userid FROM SeniorProject.Users WHERE (email = %s) AND (password = %s)), (SELECT questionaire_id FROM SeniorProject.profile AS p INNER JOIN SeniorProject.Users AS u ON u.userid = p.userid WHERE (email = %s) AND (password = %s)), %s)")
    profile_pic_answers = (email, password, email, password, profile_pic)
    cursor = cnx.cursor()
